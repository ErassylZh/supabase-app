...
type Post interface {
	...
	GetAll(ctx context.Context, postType string, filter schema.GetListingFilter) ([]model.Post, int64, error)
...
func (r *PostDB) GetAll(ctx context.Context, postType string, filter schema.GetListingFilter) (posts []model.Post, total int64, err error) {
	db := r.db.WithContext(ctx)
